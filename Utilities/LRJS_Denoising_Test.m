function [filename, Sigma,BM3D_PSNR, PSNR_Final,FSIM_Final,SSIM_Final, Time_s, iter, diffff]     =  LRJS_Denoising_Test (filename, Sigma, gamma, lambda, mu, c1, c2)
...
par              =    LRJS_Para_Set (Sigma,I, gamma, lambda, mu, c1, c2);

par.nim          =    par.I + par.nSig* randn(size( par.I ));


[BM3D_PSNR,~]       =    BM3D(par.I, par.nim, Sigma, 'np', 0);

disp(sprintf('PSNR of the BM3D Denoised image = %f \n', BM3D_PSNR));

[Denoising ,  iter, diffff]             =    LRJS_Denoising( par);
...
Final_denoisng= strcat(filename,'_LRJS','_sigma_',num2str(Sigma),'_PSNR_',num2str(PSNR_Final),'_FSIM_',num2str(FSIM_Final),'_SSIM_',num2str(SSIM_Final),'.png');
